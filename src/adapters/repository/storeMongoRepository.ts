...
export default class StoreMongoRepository
  extends AbstractMongoRepository<Store, StoreDocument, string>
  implements StoreRepository
{
  ...
  async findNearest(geoLocation: GeoLocation, limit: number): Promise<Store[]> {
    ...
    const records = await this.repositoryDocumentModel.find(filter).limit(Number(limit) || 5);
